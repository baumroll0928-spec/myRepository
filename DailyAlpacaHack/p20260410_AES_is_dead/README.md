...
0x36(=54)番地以降のピクセル部に0xffがしばらく続いています。最下段の左端の方に白色が続いているからです。
...
つまり、今回の問題のように平文画像の背景色が白色などのRGBの要素が全て同じ色一色の場合、暗号化ファイルのヘッダー部分にあたる先頭54バイトを正しいヘッダーに書き換えて画像ビューワーで開けば、キーを使って復号しなくても文字の輪郭がぼんやり浮かび上がってくることになります。
