...
* 現在のベースポインタが現在のスタックポインタ（スタックのトップの場所）で書き換えられる。（スタックが空になる。）
...
ただ、退避ベースポインタの分の8バイト＋numの分の400バイト＝408バイトだ！とかそんな単純な話ではないようです。
...
問題文の「○○は安全です！」のようにわざとらしくアピールしているところはまず疑ってかかった方がいいということを再認識しました。
