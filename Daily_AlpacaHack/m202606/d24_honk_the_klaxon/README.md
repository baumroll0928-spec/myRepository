...
満足できるまで観測したら、`CHALLENGE`を送信し、当たっていたらフラグを獲得できます。

どうすれば`CHALLENGE`を求めることができるでしょうか？
...
データを長さ256の0/1のベクトルとみなすと、(1)の変換でしているのはビットの切り出し、ビットシフト、XORだけなので、 $`y = Ax`$ のような線形変換になります。
