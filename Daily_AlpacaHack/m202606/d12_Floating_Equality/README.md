...
Pythonのfloat型の仕様はIEEE754の規格に準拠しています。

ここでIEEE754について少しだけおさらいしましょう。

小数を64ビットで表す規格ですが、その内訳は、
```
符号部(1) | 指数部(11) | 仮数部(52)
```
であり、
...
※参考までに、今回の問題をとく上で私がビット列の調査に使ったコードは[こんな感じ](show_bits.py)です。
...
仮数部のスケールで見ると、この違いはたったの $`2^{-52}`$ の違いですが、指数部も加味するとこの差は $`2^{24} = 16777216`$ まで膨れ上がることになります。
